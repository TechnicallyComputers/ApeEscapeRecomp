...
#
# Writes: dist/apeescape-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix apeescape \
  --exe-name ApeEscapeRecomp \
  --display-name "ApeEscapeRecomp" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned ApeEscape disc" \
